...
const AllTask = () => {
  ...
  const handleSaveEdit = async () => {
    await axiosSecure.put(`/task/${selectedTask._id}`, {
      title: taskTitle,
  ...
  const handleDelete = async (id) => {
    await axiosSecure.delete(`/task/${id}`);
    refetch();
  };

  const handleUpdateCategory = async (id, newCategory) => {
    await axiosSecure.put(`/task/${id}`, { category: newCategory });
    refetch();
  };
  ...
  const handleDragEnd = async (result) => {
    ...
    const newCategory = ["To-Do", "In Progress", "Done"][destination.droppableId];
    await axiosSecure.put(`/task/${draggableId}`, { category: newCategory });
    refetch();
  };
    ...
    <div className="w-11/12 mx-auto my-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(["To-Do", "In Progress", "Done"]).map((category, index) => (
            <Droppable key={category} droppableId={String(index)}>
              {(provided) => (
                <div ref={provided.innerRef} {...provided.droppableProps} className="bg-gray-100 p-4 rounded-lg shadow">
                        ...
                        <Draggable key={task._id} draggableId={task._id} index={index}>
                          {(provided) => (
                            ...
                            >
                              ...
                              <p
                                className={`mt-2 text-sm font-semibold ${task.dueDate && task.dueDate < new Date().toISOString().split("T")[0] ? "text-red-600" : "text-blue-600"}`}
                              >
                              ...
                              <div className="mt-3 flex gap-2">
                                <button
                                  ...
                                  onClick={() => handleUpdateCategory(task._id, "In Progress")}
                                >
                                  ⏳ In Progress
                                </button>
                                <button
      ...
      <dialog id="my_modal_1" className="modal">
        <div className="modal-box">
          ...
          <input type="text" value={taskTitle} onChange={(e) => setTaskTitle(e.target.value)} className="w-full p-2 mb-4 border rounded" placeholder="Task Title" />
          <textarea value={taskDescription} onChange={(e) => setTaskDescription(e.target.value)} className="w-full p-2 mb-4 border rounded" placeholder="Task Description" />
